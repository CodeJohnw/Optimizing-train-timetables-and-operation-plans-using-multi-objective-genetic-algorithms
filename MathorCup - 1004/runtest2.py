...
# 计算 valid_combinations，不再使用循环
start_end_stations = df_station[df_station['是否可作为交路起点/终点'] == '是'].index.tolist()
valid_combinations = [(sa, sb) for sa in start_end_stations for sb in start_end_stations if 3 <= sb - sa <= 24]
...
args_list = [(ga, x) for x in ga.population]
results = pool.map(parallel_objective_function, args_list)
pool.close()
pool.join()
